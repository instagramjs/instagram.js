import { createInstagramOpenAPIAxiosClient } from "@instagramjs/openapi-clients";

const axiosClient = createInstagramOpenAPIAxiosClient();

async function mainAxios() {
  const response = await axiosClient.get("/v1/direct_v2/inbox/", {
    axios: {
      headers: {
